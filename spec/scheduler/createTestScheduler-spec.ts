...
describe('TestScheduler', () => {
  describe('hotObservable', () => {
    it('should create hot observable via TestMessage', () => {
      ...
      const value = createHotObservable(messages);
    ...
    it('should create hot observable via marble diagram', () => {
      const { createHotObservable } = createTestScheduler(false, 1, 1000);
      const value = createHotObservable('---a---|');
    ...
    it('should create hot observable via marble with custom error', () => {
      const { createHotObservable } = createTestScheduler(false, 1, 1000);
      const value = createHotObservable('---a---#', null, 'meh');
    ...
    it('should honor custom frameTimeFactor', () => {
      const { createHotObservable } = createTestScheduler(false, 10, 1000);
  ...
  describe('coldObservable', () => {
    it('should create cold observable via TestMessage', () => {
      ...
      const value = createColdObservable(messages);
    ...
    it('should create cold observable via marble diagram', () => {
      const { createColdObservable } = createTestScheduler(false, 1, 1000);
      const value = createColdObservable('---a---|');
    ...
    it('should create cold observable via marble with custom error', () => {
      const { createColdObservable } = createTestScheduler(false, 1, 1000);
      const value = createColdObservable('---a---#', null, 'meh');
    ...
    it('should honor custom frameTimeFactor', () => {
      const { createColdObservable } = createTestScheduler(false, 10, 1000);
    ...
    it('should throw when marble includes unsupported token', () => {
      ...
      expect(() => createColdObservable('----^')).toThrow();
    });
  });
  ...
  describe('getMessages', () => {
    it('should generate messages from observable having hot observable source', () => {
      ...
      const { flush, getMessages, createHotObservable } = createTestScheduler(false, 1, 1000);

      const e1 = createHotObservable('---1---2---|');
    ...
    it('should able to autoflush with hot observable source', () => {
      const expected: Array<TestMessage<string>> = [next(3, 'x'), next(7, 'x'), complete(11)];
      const { createHotObservable, getMessages } = createTestScheduler(true, 1, 1000);
    ...
    it('should ignore values over max frames', () => {
      ...
      const { createHotObservable, getMessages } = createTestScheduler(true, 1, 5);
    ...
    it('should generate messages from observable having cold observable source', () => {
      const expected: Array<TestMessage<string>> = [next(3, 'x'), next(7, 'x'), complete(11)];
      const { flush, getMessages, createColdObservable } = createTestScheduler(false, 1, 1000);

      const e1 = createColdObservable('---1---2---|');
      const messages = getMessages(e1.pipe(mapTo('x')));
    ...
    it('should able to autoflush with cold observable source', () => {
      const expected: Array<TestMessage<string>> = [next(3, 'x'), next(7, 'x'), complete(11)];
      const { createColdObservable, getMessages } = createTestScheduler(true, 1, 1000);
    ...
    it('should materialize inner observable with cold observable source', () => {
      ...
      const source = createColdObservable('---a---b---c---d---|');
    ...
    it('should materialize inner observable with cold observable source with error', () => {
      ...
      const source = createColdObservable('---a---b---c---#');
    ...
    it('should materialize inner observable with hot observable source', () => {
      ...
      const source = createHotObservable('---a---b---c---d---|');
    ...
    it('should materialize inner observable with hot observable source with error', () => {
      ...
      const source = createHotObservable('---a---b---c---#');
  ...
  describe('flush', () => {
    it(`should not flush while it's already flushing`, () => {
      const { scheduler, getMessages, createHotObservable, flush } = createTestScheduler(false, 1, 1000);

      const expected = [next(20, 'a'), next(40, 'b'), next(60, 'c'), complete(80)];

      scheduler.schedule((x: typeof scheduler | undefined) => x?.flush(), 50, scheduler);

      const source = createHotObservable([next(20, 'a'), next(40, 'b'), next(60, 'c'), complete<string>(80)]);
  ...
  describe('advanceTo', () => {
    it('should throw when autoflush set', () => {
      const { advanceTo } = createTestScheduler(true, 1, 1000);

      expect(() => advanceTo(10)).toThrow();
    });

    it('should able to advance to absolute time', () => {
      const { scheduler, advanceTo, getMessages, createHotObservable } = createTestScheduler(false, 1, 1000);
      const toFrame = 50;
    ...
    it('should not do anything with zero absolute time', () => {
      const { createHotObservable, advanceTo, getMessages } = createTestScheduler(false, 1, 1000);
    ...
    it('should able to advance without any actions setup', () => {
      const { advanceTo } = createTestScheduler(false, 1, 1000);
    ...
    it('should unsubscribe the rest of the scheduled actions if an action throws an error', () => {
      const { scheduler, advanceTo } = createTestScheduler(false, 1, 1000);
