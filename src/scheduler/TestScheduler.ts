...
class TestScheduler extends VirtualTimeScheduler {
  ...
  constructor(
    ...
    private readonly frameTimeFactor: number,
  ...
  public getMessages<T = string>(
    ...
    const observableMetadata: Array<
      TestMessage<T | Array<TestMessage<T>>>
      > = [];
    const pushMetadata = (
      notification: Notification<T | Array<TestMessage<T>>>
    ) =>
      observableMetadata.push(
        new TestMessageValue<T | Array<TestMessage<T>>>(
          this.frame,
          notification
        )
      );
          ...
          pushMetadata(
            Notification.createNext(
              value instanceof Observable
                ? this.materializeInnerObservable<T>(value, this.frame)
                : value
            )
          ),
    ...
    if (this.autoFlush) {
      if (this.flushed) {
        throw new Error(
          `Cannot schedule to get marbles, scheduler's already flushed`
        );
      }
      this.flush();
    }
  ...
  public createColdObservable<T = string>(
    ...
    if (
      typeof marbleValue === "string" &&
      marbleValue.indexOf(SubscriptionMarbleToken.SUBSCRIBE) !== -1
    ) {
      throw new Error(
        `Cold observable cannot have subscription offset ${
        SubscriptionMarbleToken.SUBSCRIBE
        }`
      );
    }
    ...
    const observable = new ColdObservable<T>(
      messages as Array<TestMessage<T | Array<TestMessage<T>>>>,
      this
    );
  ...
  public createHotObservable<T = string>(
    ...
    const subject = new HotObservable<T>(
      messages as Array<TestMessage<T | Array<TestMessage<T>>>>,
      this
    );
  ...
  public advanceTo(toFrame: number): void {
    if (this.autoFlush) {
      throw new Error(
        "Cannot advance frame manually with autoflushing scheduler"
      );
    }

    if (toFrame < 0 || toFrame < this.frame) {
      throw new Error(
        `Cannot advance frame, given frame is either negative or smaller than current frame`
      );
    }
  ...
  private materializeInnerObservable<T>(
    observable: Observable<any>,
    outerFrame: number
  ): Array<TestMessage<T>> {
    const innerObservableMetadata: Array<TestMessage<T>> = [];
    const pushMetaData = (notification: Notification<T>) =>
      innerObservableMetadata.push(
        new TestMessageValue<T>(this.frame - outerFrame, notification)
      );
